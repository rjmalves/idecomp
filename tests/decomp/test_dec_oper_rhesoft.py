...
def test_eq_dec_oper_rhesoft():
    m: MagicMock = mock_open(read_data="".join(MockDecOperRheSoft))
    with patch("builtins.open", m):
        rel1 = DecOperRheSoft.read("./tests/mocks/arquivos/dec_oper_rhesoft.py")
        rel2 = DecOperRheSoft.read("./tests/mocks/arquivos/dec_oper_rhesoft.py")
        assert rel1 == rel2
...
def test_atributos_encontrados_dec_oper_rhesoft_v31():
    ...
    with patch("builtins.open", m):
        rel = DecOperRheSoft.read(
            "./tests/mocks/arquivos/dec_oper_rhesoft.py", version="31.0.2"
        )
